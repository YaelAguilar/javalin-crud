...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Pasar el ViewModel a la View para que esta pueda interactuar con él.
    const bookView = new BookView(bookViewModel);
    console.log('BookView initialized.');

    // 5. Iniciar la carga inicial de datos desde el ViewModel
    // El ViewModel se encargará de comunicarse con el Repository y luego
    // notificará a la View para que renderice los libros.
    bookViewModel.loadBooks().then(() => {
        console.log('Initial books loaded.');
    }).catch(error => {
        console.error('Failed to load initial books:', error);
        // La BookView ya mostrará la notificación de error
    });
